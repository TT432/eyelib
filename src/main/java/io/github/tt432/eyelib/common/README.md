...
- Forge subscribers, command wiring, packet sends, mob-goal inspection, and Minecraft-backed entity/event logic live under explicit runtime owners such as `mc/impl/common/entity/`.

## Current split
- `common/runtime/` is the preferred home for extracted pure update logic.
- Legacy handlers in `common/` should not accumulate new Minecraft/Forge responsibilities; prefer a named runtime observer package for event/entity wiring.
