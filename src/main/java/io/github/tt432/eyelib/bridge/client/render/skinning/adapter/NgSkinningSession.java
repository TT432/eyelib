...
/**
 * {@link SkinningSession} 的 26.1.2 实现：骨骼矩阵直接写入共享 CPU 暂存（每 phase flush 时一次性批量上传 palette ring），
...
public final class NgSkinningSession implements SkinningSession {
    ...
    public boolean begin(BakedModel model, float @Nullable [] tintColor, int overlay, int light) {
        ...
        int offset = NgSkinningManager.acquirePalette(geo.paletteBlockSize());
        if (offset < 0) {
            return false;
        }
        this.geometry = geo;
        ...
        ByteBuffer buf = NgSkinningManager.paletteStaging();
        buf.putFloat(offset, r).putFloat(offset + 4, g).putFloat(offset + 8, b).putFloat(offset + 12, a);
        buf.putInt(offset + 16, overlay & 0xFFFF).putInt(offset + 20, overlay >>> 16);
        buf.putInt(offset + 24, light & 0xFFFF).putInt(offset + 28, light >>> 16);
    ...
    public void appendBone(int boneId, Matrix4f pose, Matrix3f normal) {
        if (!active || geometry == null || paletteOffset < 0) {
            return;
        }
        ...
        ByteBuffer buf = NgSkinningManager.paletteStaging();
        pose.get(paletteOffset + SkinningLayout.poseOffset(slot), buf);
        // 法线阵写入 mat4 slot 的上左 3×3（列主序；w 列/行不被 mat3() 读取，置 0 防 NaN 传播）
        int base = paletteOffset + SkinningLayout.normalOffset(slot, geometry.slotCount());
    ...
    public void finish() {
        if (!active || geometry == null || paletteOffset < 0 || visible == null) {
            return;
        }
        ranges = geometry.coalesceVisible(visible);
        paletteSlice = NgSkinningManager.paletteSlice(paletteOffset, geometry.paletteBlockSize());
        if (ranges.length == 0) {
