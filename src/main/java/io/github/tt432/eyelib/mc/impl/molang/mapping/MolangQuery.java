...
public final class MolangQuery {
    ...
    public static float isCharging(MolangScope scope) {
        return scope.getHostContext().get(Vex.class).map(v -> v.isCharging() ? TRUE : FALSE)
    ...
    public static float isTamed(MolangScope scope) {
        return scope.getHostContext().get(TamableAnimal.class).map(TamableAnimal::isTame).orElse(false) ? TRUE : FALSE;
    }
    ...
    public static float isItemEquipped(MolangScope scope, String hand) {
        return scope.getHostContext().get(LivingEntity.class).map(living -> (switch (hand.toString()) {
    ...
    public static float isEmerging(MolangScope scope) {
        return scope.getHostContext().get(Warden.class).map(w -> w.hasPose(Pose.EMERGING)).orElse(false) ? TRUE : FALSE;
    }
    ...
    public static float dashCooldownProgress(MolangScope scope) {
        return scope.getHostContext().get(Camel.class).map(c -> c.getJumpCooldown() / 55F).orElse(0F);
    }

    @MolangFunction(value = "is_saddled", description = "正在背负")
    public static float isSaddled(MolangScope scope) {
        return scope.getHostContext().get(AbstractHorse.class).map(AbstractHorse::isSaddled).orElse(false) ? TRUE : FALSE;
    }

    @MolangFunction(value = "is_sheared", description = "被剪了")
    public static float isSheared(MolangScope scope) {
        return scope.getHostContext().get(Sheep.class).map(Sheep::isSheared).orElse(false) ? TRUE : FALSE;
    }

    @MolangFunction(value = "is_angry", description = "生气")
    public static float isAngry(MolangScope scope) {
        return scope.getHostContext().get(NeutralMob.class).map(NeutralMob::isAngry).orElse(false) ? TRUE : FALSE;
    }

    @MolangFunction(value = "is_carrying_block", description = "抱方块")
    public static float isCarryingBlock(MolangScope scope) {
        return scope.getHostContext().get(EnderMan.class).map(e -> e.getCarriedBlock() != null).orElse(false) ? TRUE : FALSE;
    }

    @MolangFunction(value = "is_chested", description = "")
    public static float isChested(MolangScope scope) {
        return scope.getHostContext().get(AbstractChestedHorse.class).map(AbstractChestedHorse::hasChest).orElse(false) ? TRUE : FALSE;
    }
    ...
    public static float timeSinceLastVibrationDetection(MolangScope scope) {
        return scope.getHostContext().get(Warden.class).map(w -> (40L - w.getBrain().getTimeUntilExpiry(MemoryModuleType.VIBRATION_COOLDOWN)) / 20F).orElse(0F);
    }

    @MolangFunction(value = "heartbeat_phase", description = "心跳（监守者）")
    public static float heartbeatPhase(MolangScope scope) {
        return scope.getHostContext().get(Warden.class).map(w -> w.getHeartAnimation(partialTicks(scope))).orElse(0F);
    }
    ...
    public static float isPowered(MolangScope scope) {
        return scope.getHostContext().get(PowerableMob.class).map(c -> c.isPowered() ? TRUE : FALSE).orElse(FALSE);
    }
    ...
    public static float creeperSwell(MolangScope scope) {
        return scope.getHostContext().get(Creeper.class).map(c -> c.swell / 30F)
                .orElse(Float.valueOf(scope.getHostContext().get(WitherBoss.class).map(WitherBoss::getInvulnerableTicks).orElse(0)));
    }

    @MolangFunction(value = "swelling_dir", description = "苦力怕爆炸方向")
    public static float swellingDir(MolangScope scope) {
        return scope.getHostContext().get(Creeper.class).map(Creeper::getSwellDir).orElse(0);
    }

    @MolangFunction(value = "invulnerable_ticks", description = "")
    public static float invulnerableTicks(MolangScope scope) {
        return scope.getHostContext().get(WitherBoss.class).map(WitherBoss::getInvulnerableTicks).orElse(0);
    }
    ...
    public static float headXRotation(MolangScope scope, float head) {
        return scope.getHostContext().get(WitherBoss.class).map(w -> w.getHeadXRot((int) head)).orElse(0F);
    }
    ...
    public static float isStanding(MolangScope scope) {
        return scope.getHostContext().get(AbstractHorse.class).map(AbstractHorse::isStanding).orElse(false) ? TRUE : FALSE;
    }

    @MolangFunction(value = "shake_angle", description = "头摇晃角度（狼）")
    public static float shakeAngle(MolangScope scope) {
        return scope.getHostContext().get(Wolf.class).map(w -> w.getHeadRollAngle(partialTicks(scope))).orElse(0F);
    }
    ...
    private static float entityFloat(MolangScope scope, Function<Entity, Float> function) {
        return scope.getHostContext().get(Entity.class).map(function).orElse(0F);
    }
    ...
    private static float handIs(MolangScope scope, InteractionHand hand, String... objects) {
        return scope.getHostContext().get(LivingEntity.class).map(living -> {
