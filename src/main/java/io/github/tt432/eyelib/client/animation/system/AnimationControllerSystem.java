...
public class AnimationControllerSystem {
    final List<Animatable<?>> entities = new ArrayList<>();

    public void register(Animatable<?> animatable) {
        entities.add(animatable);
    }

    public void update(float aTick) {
        for (Animatable<?> entity : entities) {
            AnimationComponent animationComponent = entity.animationComponent();
            ...
            if (animTick > currentAnimation.getAnimationLength()) {
                animationComponent.updateStartTick(aTick);
            }

            ModelComponent modelComponent = entity.modelComponent();
    ...
    private static void updateBoneAnimation(BrAnimationEntry currentAnimation, float animTick, ModelComponent modelComponent) {
        for (var boneAnimation : currentAnimation.getBones().entrySet()) {
            BrBoneAnimation value = boneAnimation.getValue();
            BrBone bone = modelComponent.getModel().getAllBones().get(boneAnimation.getKey());
