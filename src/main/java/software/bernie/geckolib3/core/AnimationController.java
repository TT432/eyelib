...
import io.github.tt432.eyelib.api.SoundPlayer;
import io.github.tt432.eyelib.api.model.Bone;
import io.github.tt432.eyelib.util.BoneSnapshot;
import io.github.tt432.eyelib.util.math.easing.EasingManager;
...
public class AnimationController<T extends Animatable> {
    ...
    private final Map<ResourceLocation, Boolean> currentAnimSoundCache = new HashMap<>();
    ...
    protected Queue<AnimationEntry> animationQueue = new LinkedList<>();
    protected AnimationEntry currentAnimation;
    ...
    public void setAnimation(AnimationBuilder builder) {
        ...
        if (model != null) {
            if (builder == null || builder.getRawAnimationList().isEmpty()) {
                ...
                LinkedList<AnimationEntry> animations = builder.getRawAnimationList().stream().map(rawAnimation -> {
                    AnimationEntry animation = model.getAnimation(rawAnimation.animationName, animatable);
    ...
    public void process(final double tick, AnimationEvent<T> event, List<Bone> modelRendererList,
        ...
        if (this.currentAnimation != null) {
            ...
            if (model != null) {
                AnimationEntry animation = model.getAnimation(currentAnimation.getAnimationName(), this.animatable);
    ...
    private void saveSnapshotsForAnimation(AnimationEntry animation,
    ...
    private void processCurrentAnimation(double tick, double actualTick, MolangParser parser,
        ...
        if (tick >= this.currentAnimation.getAnimationLength()) {
            ...
            if (!this.currentAnimation.getLoop().isRepeatingAfterEnd()) {
                ...
                AnimationEntry peek = this.animationQueue.peek();
        ...
        for (var entry : boneAnimations.entrySet()) {
            ...
            io.github.tt432.eyelib.animation.pojo.BoneAnimation boneAnim = entry.getValue();
        ...
        processSoundEffect(event, tick);

        if (this.transitionLengthTicks == 0 && shouldResetTick && this.animationState == AnimationState.TRANSITIONING)
            this.currentAnimation = animationQueue.poll();
    }

    private void processSoundEffect(AnimationEvent<T> event, double tick) {
        if (event.getAnimatable() instanceof SoundPlayer sp) {
            SoundManager soundManager = Minecraft.getInstance().getSoundManager();

            for (var entry : currentAnimation.getSoundEffects().entrySet()) {
                Timestamp timestamp = entry.getKey();
                var sounds = entry.getValue().getEffect();

                for (var sound : sounds) {
                    if (tick >= timestamp.getTick() && !Boolean.TRUE.equals(currentAnimSoundCache.get(sound))) {
                        soundManager.play(sp.getSound(sound));
                        currentAnimSoundCache.put(sound, true);
                    }
                }
            }
        }
