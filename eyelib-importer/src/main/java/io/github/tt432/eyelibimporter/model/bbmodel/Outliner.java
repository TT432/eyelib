...
public record Outliner(
    ...
    public static final Codec<Outliner> CODEC = BbModelCodecs.recursive("Outliner", self ->
            RecordCodecBuilder.create(ins -> ins.group(
                    Codec.STRING.fieldOf("uuid").forGetter(Outliner::uuid),
                    BbModelCodecs.optionalMapCodec(Group.MAP_CODEC).forGetter(Outliner::group),
                    Codec.BOOL.fieldOf("isOpen").forGetter(Outliner::isOpen),
                    BbModelCodecs.withAlternative(
                                    self.xmap(
