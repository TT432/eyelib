...
- Its Gradle artifact is registered as the `eyelibimporter` Forge mod through `META-INF/mods.toml` and `mc/impl/bootstrap/EyelibResourcesImporterMod.java`.
...
- Keep Minecraft/Forge bootstrap isolated to `mc/impl/bootstrap/` unless a future importer-owned runtime integration explicitly needs more Forge wiring.
...
- If new Minecraft/Forge imports are needed outside `mc/impl/bootstrap/`, document why they are importer-owned functional behavior before adding them.
