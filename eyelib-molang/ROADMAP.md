...
- `compiler/cache/MolangCompileCache.java` provides two-tier caching of compiled expressions: L1 in-memory (ConcurrentHashMap) + L2 persistent disk (MolangDiskCache).
- `compiler/cache/MolangDiskCache.java` provides persistent disk-based bytecode caching with `.molcache` binary format — magic number header validation, compiler-version and mapping-registry-version staleness detection, CRC32 corruption detection, and atomic write-temp-rename semantics. Integrated as L2 (disk) tier in `MolangCompileCache`.
