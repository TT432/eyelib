...
## Summary
- Eyelib is a multi-project `Gradle + Java 17 + Forge` repository: root runtime module plus processor subproject `eyelib-processor`, importer/model subproject `eyelib-importer`, engine Molang subproject `eyelib-molang`, and particle boundary subproject `eyelib-particle`.
...
| eyelib-particle subproject | Particle module API and core contract boundary skeleton for future particle extraction, with Forge-visible metadata but no migrated runtime behavior in Phase 8 | `eyelib-particle/build.gradle`, `eyelib-particle/src/main/java/io/github/tt432/eyelibparticle/`, `eyelib-particle/src/main/resources/META-INF/mods.toml`, `eyelib-particle/src/test/` | consumed directly by root through one-way project dependencies; must not depend back on root runtime packages, managers, registries, packets, capability helpers, or `mc/impl` classes; current root particle runtime remains under `src/main/java/io/github/tt432/eyelib/client/particle/` until later phases move it through explicit seams |
