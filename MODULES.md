...
## Summary
- Eyelib is a multi-project `Gradle + Java 17 + Forge` repository: root runtime module plus processor subproject `eyelib-processor`, importer/model subproject `eyelib-importer`, and engine Molang subproject `eyelib-molang`.
...
| External reference docs | source-backed Chinese reference notes for external Bedrock addon/container/pack facts and structures | `docs/reference/` | records official docs, community references, and real-sample findings for addon research |
...
| Molang roadmap and refactor plan docs | current-state Molang roadmap plus dependency-ordered rewrite plan, entry gates, parity matrix, and cutover guidance | `eyelib-molang/ROADMAP.md`, `eyelib-molang/refactor-plan/README.md`, `eyelib-molang/refactor-plan/*.md` | `ROADMAP.md` is the current-state source of truth and must be updated when Molang phase status, milestones, gates, ownership, verification posture, or cutover posture changes; phase plans provide detailed execution gates after the roadmap |
...
| eyelib-material subproject | Bedrock material definitions, GL state management, and shader pipeline | `eyelib-material/build.gradle`, `eyelib-material/src/main/java/io/github/tt432/eyelibmaterial/` | consumed by root render controllers, particle renderer, and `:eyelib-material-smoke`; depends on no other eyelib subprojects |
| Client smoke framework | External standalone client-side smoke discovery, annotation API, world setup, screenshot/report output, mouse-release support, and visual hook API | `clientsmoke/` Git submodule | supplied to this repository through Gradle composite build `includeBuild("clientsmoke")`; loaded by root `clientSmoke` run config; scans `@ClientSmoke` tests from runtime modules; must not depend on feature modules such as `:eyelib-material` |
| eyelib-material smoke target | Material-specific client smoke test mod and shader resources for visual framebuffer verification | `eyelib-material-smoke/build.gradle`, `eyelib-material-smoke/src/main/java/io/github/tt432/eyelibmaterialsmoke/`, `eyelib-material-smoke/src/main/resources/` | loaded only by root `clientSmoke` run config; depends on composite-build module `io.github.tt432:clientsmoke` and `:eyelib-material` so the generic smoke framework remains module-agnostic |
...
| Client entity runtime | runtime-facing client-entity and attachable lookup/helpers plus RenderData-side client-entity component support over importer-owned client-entity/attachable schema | `src/main/java/io/github/tt432/eyelib/client/entity/`, `src/main/java/io/github/tt432/eyelib/client/entity/README.md` | interacts with importer-owned parsing/schema, client-entity and attachable manager stores, RenderData capability components, render controllers, and particles |
| Client entity lookup seam | narrow runtime read access to client entities and attachables through platform-type-free identifiers | `src/main/java/io/github/tt432/eyelib/client/entity/ClientEntityLookup.java`, `src/main/java/io/github/tt432/eyelib/client/entity/AttachableLookup.java` | used by render/runtime setup, with Minecraft id adaptation kept at MC-facing call sites |
...
| Client material domain | material definitions and entries | `eyelib-material/src/main/java/io/github/tt432/eyelibmaterial/` | used by render controllers and material manager |
...
| Asset loaders | own reload orchestration and root-side adaptation for animations, materials, particles, entities, attachables, and render controllers while importer-owned schema parsing migrates into `:eyelib-importer` | `src/main/java/io/github/tt432/eyelib/client/loader/`, `src/main/java/io/github/tt432/eyelib/client/loader/README.md` | publish into domain-specific registry writers; loader registration lifecycle is bound in `mc/impl/client/loader/`; platform-free parse/translate helpers now come from `:eyelib-processor`; client-entity/attachable codecs and controller-set parsing now come from `:eyelib-importer`, while animation-entry parsing is still transitional |
...
| Runtime managers | singleton manager stores for animations, models, materials, particles, render controllers, client entities, and attachables | `src/main/java/io/github/tt432/eyelib/client/manager/` | consumed through lookup seams and domain-specific writer classes; manager entry-change publication routes through `mc/api` bridge |
| Client registry seam | domain-specific loader/tooling→store publication boundaries, including attachable publication alongside client entities and addon bridge consumers | `src/main/java/io/github/tt432/eyelib/client/registry/`, `src/main/java/io/github/tt432/eyelib/client/registry/README.md` | used by loaders, manager import planner, manager import actions, and addon runtime bridging; replacement seams are string-keyed where possible to avoid platform identifier leakage |
...
| Manager screen reload seam | resource-folder and Bedrock addon import plus file watch lifecycle for animations, controllers, particles, entities, attachables, Bedrock models, and textures; runtime publication/upload orchestration stays in root while platform-free route/texture-key/batch helpers move to `:eyelib-processor` | `src/main/java/io/github/tt432/eyelib/client/gui/manager/reload/` | publishes through domain-specific registry seams, holds monitored folder session state, consumes addon aggregates through root bridge helpers, delegates plain planning/batching to `:eyelib-processor`, and should avoid owning importer normalization logic |
...
| Molang engine scope/compiler/type | Engine-owned scope, compilation, persistent disk cache (MolangDiskCache with header validation), diagnostic/bytecode emission, and Molang type system | `eyelib-molang/src/main/java/io/github/tt432/eyelibmolang/MolangScope.java`, `eyelib-molang/src/main/java/io/github/tt432/eyelibmolang/compiler/`, `eyelib-molang/src/main/java/io/github/tt432/eyelibmolang/compiler/cache/`, `eyelib-molang/src/main/java/io/github/tt432/eyelibmolang/compiler/cache/MolangDiskCache.java`, `eyelib-molang/src/main/java/io/github/tt432/eyelibmolang/compiler/diagnostic/`, `eyelib-molang/src/main/java/io/github/tt432/eyelibmolang/type/` | exposed to root through `project(':eyelib-molang')`; no reverse dependency back into root runtime |
