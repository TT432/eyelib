...
唯一可行路径: Windows 侧 Python(WSL 下 GLES 3.x 不支持 replay)。
...
5. 调用 Python 脚本: `& "C:\Users\<user>\AppData\Local\Programs\Python\Python313\python.exe" E:\path\to\script.py`
6. 先 `write_file` 写 Python 脚本再调用,避免 shell 转义

## renderdoc-mcp：Headless .rdc 分析

[renderdoc-mcp](https://pypi.org/project/renderdoc-mcp/) 捆绑 renderdoc.so，通过 MCP 协议提供 42 个 headless 分析工具，无需 GUI。

已在 opencode 的 `mcp_servers.renderdoc` 中配置，工具以 `mcp_renderdoc_*` 前缀注册。
