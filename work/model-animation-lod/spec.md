...
- 保留开发态 `V` 键入口与 `.bbmodel` 拖入；拖入模型包装为临时 ClientEntity 后走同一渲染路径。
- 从 `ClientEntityManager` 当前快照中按 identifier 搜索 ClientEntity；输入时显示匹配数，按 Enter 或 Select 明确选择，并提供上一项/下一项操作。
- 预览使用隔离的 `RenderData`，以本地玩家作为 Molang host，复用生产 `setupClientEntity`、render controller、material、纹理合成/alpha clamp、part visibility 与动画姿势求值；禁止从 geometry 名猜测纹理或绕过 render controller 直接渲染 Model。
- 提供 `LOD Strength: 0%..100%` 滑条；修改时立即更新生产 LOD 控制器的全局强度，并在预览 ClientEntity 上以模拟 px/unit 可视化几何剔除强度。
- 界面显示当前 ClientEntity、实际 render-controller component 数量、实际纹理、强度、预览 LOD 级别、模拟 px/unit、旋转/缩放/平移。
- 顶点统计按 render-controller component 的 part visibility 与材质分组计算，显示 `FULL 顶点数 -> 当前 LOD 顶点数` 和减少百分比。
- 搜索、ClientEntity setup 或渲染失败时显示明确状态，不终止客户端。
...
- 预览屏幕选择 ClientEntity 后，其 ModelComponent 列表、render pass、纹理与生产 setup 流程一致；滑条值与全局强度一致。
...
- ClientEntity 无几何或 render controller 无有效输出：可选择但不提交顶点，界面显示 0 个 RC component，不崩溃。
- ClientEntity 引用的纹理或材质缺失：遵循生产 render controller 的 missing texture/fallback 语义，不做 debug 专用猜测。
- 搜索空字符串：不改变当前选择并提示输入 ClientEntity identifier。
- 非法强度（NaN/无穷/区间外）：规范化到 `[0,1]`，NaN 视为 `0`。
- setup、bake 或渲染失败：保留界面并显示错误消息，不得终止客户端。
