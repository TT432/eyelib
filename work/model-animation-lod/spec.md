...
- 从 `ModelManager` 当前快照中按名称/ID搜索模型；输入时显示匹配数，按 Enter 或 Select 明确选择，并提供上一项/下一项操作。
- 通过 `ClientEntityManager` 的 geometry/texture 表为模型匹配实际动态纹理；常见 vanilla 模型尝试资源路径推导；无法解析时显示 missing texture，不伪装成无纹理灰色模型。
- 提供 `LOD Strength: 0%..100%` 滑条；修改时立即更新生产 LOD 控制器的全局强度，并在预览模型上以模拟 px/unit 可视化几何剔除强度。
- 界面显示当前模型、纹理、强度、预览 LOD 级别、模拟 px/unit、旋转/缩放/平移。
- 界面显示 `FULL 顶点数 -> 当前 LOD 顶点数` 和减少百分比。
...
- 模型无纹理映射：尝试常见 vanilla 纹理路径；仍无法解析时使用 missing texture，并明确显示纹理位置。
- 搜索空字符串：不改变当前选择并提示输入名称或 ID。
