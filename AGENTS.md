...
- 不要碰无关的未提交变更。
- **临时产物处置：该删的删、该合的合，不允许用 `.gitignore` 把垃圾留在工作区**（用户纠正 2026-08-13）：调试/研究临时产物（反编译源码、截图、探针脚本、RenderDoc capture、dump 日志）用途结束后删除；有复用价值的知识合入 `docs/` 或对应 skill；`work/` 下只保留进行中工作的跟踪文档。
- **节点图编辑器全版本统一 LDLib2。** ldlib1（LDLib 1.x graphprocessor 适配层）已于 2026-08-07 退役（用户决策，见 docs/decisions/0028-ldlib1-retirement.md）：1.20.1 由 TT432/LDLib2-1.20.1-Forge fork 支撑（以 `third_party/LDLib2` submodule 维护，分支 update-2.2.34；仅自维护，不向上游发 issue/PR——用户决策 2026-08-13）。编辑器新功能只需在 ldlib2 适配层单端实现。
- **UI 特性必须验证到渲染层**（用户纠正 2026-08-08）：域模型/选项/端口定义存在 ≠ 编辑器实际渲染。验收编辑器特性时必须在运行客户端确认 LDLib2 模型层与视觉呈现；理解需求中的参照术语（如「图化」）时先核对参照物的完整形态再动手。
- **第三方库需要修补时，交付方案必须包含源码级修复的归属**（上游 PR 或 fork），本地补丁 jar 只能作为过渡并显式标注（用户纠正 2026-08-06）。
- **「X 编辑器里某行为要改」类需求默认落在 eyelib 自身编辑器代码（调用/配置层）**（用户纠正 2026-08-17）：先评估 LDLib2 style/配置层能否满足；改 `third_party/LDLib2` 库本身是最后手段且必须先与用户确认。反例：LOD_SIMPLIFIED_PIXEL_SCALE 限界事件——编辑器侧 `Ldlib2Workbench` style 一行可解决，却改了库（已回退，库内 fa4e0401 / 根仓库 5715fe00+7444184d）。
- 优先做窄幅编辑，避免大范围的包结构调整。
