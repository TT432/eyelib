...
## 5. 已知限制
## 6. Opt19 后大规模复测（2026-08-29）：n700 回退已消除

C1 初测（Pre-Opt16~19）曾观察 n700/896 规模 ON 明显回退，当时归因为逐实体 draw+状态机开销。
Opt19（Setup/Tick 跨实体并行化）+ Opt16~18（molang/map 机制消除）后同机复测
（world slime，A&S 包，45s 测量，串行协议，A/B 仅切 `-Deyelib.gpuSkinning`）：

| n | ON | OFF | 判定 |
|---|---|---|---|
| 700 | 43.54 / 40.60 | 36.61 / 37.18 | **ON +9~19%，区间不重叠** |
| 896 | 34.25 / 37.13 | 35.34 / 34.78 | 中性（重叠，±4% 方差） |

结论：回归已被后续优化消除——原回归的真实成因是渲染线程上 CPU 侧工作（molang 求值/setup）
饱和后逐实体提交开销成为压死项；该 CPU 工作移出渲染线程后，GPU 蒙皮免去逐顶点 CPU 变换
与每帧全量顶点上传的优势在 n700 规模兑现。**无需阈值回退**。benchmark 目录资源包选择已核实
（versions/26.1.2/run/benchmark/options.txt 含 Actions-and-Stuff-1.10-v2.mcpack）。
