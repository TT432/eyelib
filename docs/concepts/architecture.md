...
1. **Domain 模块包不 import `net.minecraft.*` / `net.minecraftforge.*`**。
   - 历史状态:ADR-0010 原本由 ArchUnit 强制;ADR-0014 因收益不抵成本删除;ADR-0015 以 freeze 模式恢复。
   - 当前状态:**ArchUnit 已落地**（freeze 模式）。`ArchitectureTest.java` 按 ADR-0016 四层模型约束 domain 包零 MC import;`IqfSourceScanRulesTest.java` 按 ADR-0018 约束 IQF 规则。baseline 存于 `build/archunit_store/`,新违规直接 fail,老违规按 Phase 3 逐步还债(见 [ADR-0015](../decisions/0015-stonecutter-multi-version.md) §4)。
