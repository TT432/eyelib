...
- Expected final state for this module: packet transport, side gating, and player/entity-backed dispatch remain in `mc/impl`; feature-specific packet contracts belong to their owning feature modules, and only transport-independent shared contracts, payload DTOs, and apply ports may survive outside `mc/impl`.
...
- Packet contract classes no longer remain under legacy `network/`; particle packet contracts now live under `:eyelib-particle`, while non-particle attachment/render/model packet contracts remain under `mc/impl/network/packet/`. Final cleanup still has attachment/render/model ownership work left because some packet DTOs continue to use `FriendlyByteBuf` / `CompoundTag` inside root `mc/impl`.
...
- Narrowed particle packet contract ownership: particle-owned `SpawnParticlePacket` now carries a platform-type-free `String particleId` instead of `ResourceLocation`, with conversion no longer needed at command send/runtime apply call sites.
- Relocated particle packet DTO classes from `mc/impl/network/packet/` to `eyelib-particle/src/main/java/io/github/tt432/eyelibparticle/network/`, so particle protocol ownership lives with the particle functional module.
...
- Remaining sync blockers are now implementation-side packet payload concerns under `mc/impl/network/packet/**`:
  - `FriendlyByteBuf` in `AnimationComponentSyncPacket`, `DataAttachmentUpdatePacket`, `DataAttachmentSyncPacket`, `ExtraEntityDataPacket`, `ExtraEntityUpdateDataPacket`, `ModelComponentSyncPacket`, `UniDataUpdatePacket`, `UpdateDestroyInfoPacket`.
