...
| `network-sync` | functional ownership cleanup advanced | `network/`, `network/dataattach/`, `mc/impl/network/`, `mc/impl/network/packet/`, `eyelib-particle/src/main/java/io/github/tt432/eyelibparticle/network/` | channel/context/side-gating and entity/player dispatch live in `mc/impl/network`; particle packet DTO/codecs moved to `:eyelib-particle`, while remaining root packet follow-up is attachment/render/model payload ownership under `mc/impl/network/packet/` |
...
| `client-managers-registry` | functional ownership cleanup advanced | `client/manager/`, `client/registry/` | manager entry publication and Forge event-bus publisher are now owned by `client/manager`; client-entity registry replacement seam no longer exposes `ResourceLocation`; scoped rule-based scans passed, while fresh full-module MCP confirmation still depends on a stable validation session |
...
| `client-particle` | completed (hard-import slice advanced) | `client/particle/`, `eyelib-particle/src/main/java/io/github/tt432/eyelibparticle/api/` | spawn/request seam tightened: module API `ParticleSpawnRequest` is platform-type-free (`String` ids + state copy), with packet adaptation kept in `ParticleSpawnService`; heavy particle runtime/render owners remain queued for `mc/impl` quarantine |
...
| `utility-mc-bridges` | final isolation in progress (slice advanced) | `util/client/`, `util/codec/`, selected `util/math/` | wave 2 timer + modbridge quarantine is complete, wave 3 moved/removed legacy util helper shims, and wave 4 deleted dead `util/client` runtime bridge wrappers (`BakedModels`, `BlitCall`, `BufferBuilders`); modbridge module (`BBModelSink`, `ModBridgeServer`, `ModBridgeModelUpdateEvent`) was deleted as dead code; remaining blockers are now concentrated in `util/codec/*` runtime codecs plus `util/ResourceLocations` and `util/math/Shapes`, and this retry has fresh JetBrains MCP confirmation (`test --tests io.github.tt432.eyelib.core.util.time.FixedStepTimerStateTest` + project build both pass) |
...
- `network-sync` particle packet DTO classes are now under `eyelib-particle/src/main/java/io/github/tt432/eyelibparticle/network/`; remaining root packet follow-up is attachment/render/model payload ownership under `mc/impl/network/packet/`.
...
- The `mc/` root is now treated as transitional root wiring, not a functional boundary. Current cleanup moves feature-owned code back to its feature module even when it uses Minecraft/Forge APIs.
- `mc/impl` should no longer consolidate by platform import alone; remaining subtrees need functional ownership review before more code is added there.
- `mc/api` is obsolete as a destination for new contracts; the manager publisher contract has moved to `client/manager`.
- Utility hard-quarantine advanced beyond first wave: fixed-step timer arithmetic now lives in plain-JVM `core/util/time/FixedStepTimerState`, the temporary Minecraft runtime `FixedTimer` adapter was removed after particle callers moved to module-owned `ParticleTimer`, Forge `modbridge` update event was moved to `mc/impl/modbridge/ModBridgeModelUpdateEvent` and later deleted entirely (dead code), and legacy util helper shims were further drained (`client/render/PoseCopies` owner move, inventory model helper move to `mc/impl/util/model`, obsolete util facade deletion); remaining utility blockers are MC-facing codec/runtime helpers under `util/codec/*`.
- `FixedStepTimerState` has a local semantic fix for the first-step accounting bug (`start()` resets `init`, first immediate step advances `lastFixed`) and remains covered by `FixedStepTimerStateTest`.
- `client-managers-registry` is now functionally owned for this scope: manager storage remains behind `ManagerStorage<T>`, lookup/publish call sites use `ManagerReadPort<T>`/`ManagerWritePort<T>`, manager event publication classes now live in `client/manager`, and `ClientEntityAssetRegistry` replacement no longer leaks `ResourceLocation`; fresh full-module MCP confirmation still needs a stable validation session.
- `client-loaders` hard-import quarantine slice advanced: concrete `Br*Loader` classes no longer own Forge reload-listener registration, with registration centralized under `mc/impl/client/loader/ClientLoaderLifecycleHooks`; preprocessing-owned `eyelibpreprocessing.loader.LoaderParsingOps` now uses source-key generics instead of `ResourceLocation` in parse/translation method signatures, and preprocessing seam tests now validate plain string-key behavior. JetBrains MCP verification is only partially available in this session: boundary scans and loader-scope code changes are confirmed, but some MCP operations (`get_file_problems`, `run_gradle_tasks`, `build_project`) were unavailable and run-config status has shifted across slices.
...
- `network-sync` functional ownership slice advanced: transport/channel/context/side-gating ownership now lives under `mc/impl/network` (`EyelibNetworkTransport` + `DataAttachmentSyncRuntime`), `NetClientHandlers` is context-free, `UniDataUpdatePacket` no longer exposes `RegistryObject`, and particle-owned `SpawnParticlePacket` no longer exposes `ResourceLocation` (string-keyed particle id contract); remaining packet debt is concentrated in non-particle attachment/render/model packet ownership under `mc/impl/network/packet/**`.
...
- `client-gui-tools` hard-import slice advanced with minimal churn: preprocessing-owned `eyelibpreprocessing.manager.reload.ManagerResourceReloadPlan` now owns normalized single-file route classification plus string-key texture planning (`Path`/`String` only), `ManagerResourceImportPlanner` delegates both route and texture-key decisions and no longer constructs `ResourceLocation` directly, targeted seam tests were expanded under `:eyelib-preprocessing`, and runtime UI/hotkey/preview owners remain queued for `mc/impl` relocation.
...
- `client-particle` hard-import slice advanced from first-wave baseline: module API `io.github.tt432.eyelibparticle.api.ParticleSpawnRequest` is platform-type-free (string-keyed particle id + defensive position copy, no direct packet/`ResourceLocation` type coupling), the obsolete root duplicate request type has been removed, and packet adaptation is explicit at `ParticleSpawnService` runtime wiring; targeted seam test and JetBrains project build checks pass, while `BrParticleRenderManager`, emitters/particles, render hooks, and level/camera/render-type access remain MC-facing and still queued for `mc/impl` relocation.
