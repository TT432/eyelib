...
- `eyelib-particle/src/main/java/io/github/tt432/eyelibparticle/`: first-class particle module boundary, build/package contract, module-owned particle APIs, `io.github.tt432.eyelibparticle.runtime.ParticleDefinition` as the canonical module runtime definition owner, `ParticleDefinitionAdapter` as the explicit adapter seam without root runtime reverse dependency, Phase 11 executable runtime/render-manager/client integration ownership, Phase 12 loading/publication ownership through `ParticleDefinitionRegistry`, `ParticleResourcePublication`, and the active `ParticleStore<ParticleDefinition>` keyed by `ParticleDefinition.identifier()`, while Phase 13 command/network integration enters only through documented root/MC adapters.
...
- `src/main/java/io/github/tt432/eyelib/network/`: packet registration entrypoint and context-free client/server handler delegation; Phase 13 particle packet DTO/codecs live under `mc/impl/network/packet`.
...
- Client-gui-tools hard-quarantine follow-up has started from reload planning state: platform-free path normalization, single-file route classification, and texture-key planning (`Path`/`String` only) now live in `:eyelib-processor` (`io.github.tt432.eyelibprocessor.manager.reload.ManagerResourceReloadPlan`), while root `ManagerResourceImportPlanner` keeps Minecraft/Forge texture upload and event posting runtime wiring and now delegates plain file-batch assembly to `:eyelib-processor`.
...
| `client/registry/` | `client.registry` | Hold domain-specific loader/tooling-to-manager publication seams; `ParticleAssetRegistry` is compatibility-only and delegates active particle writes to `ParticleDefinitionRegistry.publisher()` |
...
| `:eyelib-particle/**` | `client.particle.module` | Own particle module contracts, `io.github.tt432.eyelibparticle.runtime.ParticleDefinition` as the canonical module runtime definition owner, `ParticleDefinitionAdapter` as the named conversion seam from importer-owned schema, executable runtime components/lifecycle, render-manager behavior, render adapter behavior, `Dist.CLIENT` client hook integration, and Phase 12 active loading/publication through `ParticleDefinitionRegistry` plus `ParticleResourcePublication`; root may consume it, but it must not depend on root runtime packages, root managers, root registries, root packets, root capability helpers, or root mc/impl; `io.github.tt432.eyelibimporter.particle.BrParticle` is the canonical raw Bedrock particle schema/codec owner, the allowed particle -> importer dependency for ParticleDefinitionAdapter is the only schema dependency direction, and active publication parses importer-schema JSON through `ParticleResourcePublication`, converts with `ParticleDefinitionAdapter`, and stores by `ParticleDefinition.identifier()` in the module `ParticleStore<ParticleDefinition>`; retained root particle classes (`BrParticleLoader`, `ParticleAssetRegistry`, `ParticleManager`, `ParticleLookup`, `ParticleSpawnService`, and root `client/particle/bedrock/BrParticle`) are transitional compatibility adapters only and must not own canonical loading/publication business logic; Minecraft/Forge client rendering and `:eyelib-material` render-type resolution are allowed only in the documented particle `client/**` integration layer; Phase 13 rewires command/network integration, and Phase 14 owns broad/client verification evidence |
| `network/` | `sync` | Own network entrypoint and context-free handler delegation; `NetClientHandlers` stays transport/context-free and routes particle spawn/remove packets to `ParticleSpawnService` without render-manager, loader, or registry internals |
| `mc/impl/network/packet/` | platform integration zone | Own Phase 13 particle packet DTO/codecs, including `SpawnParticlePacket(String spawnId, String particleId, Vector3f position)` and `RemoveParticlePacket(String removeId)`, `FriendlyByteBuf`, and string encode/decode calls; PFUT-02 packet-contract relocation remains deferred |
...
- Root `ParticleAssetRegistry`, `ParticleManager`, `ParticleLookup`, and `ParticleSpawnService` are named compatibility adapters. They may bridge existing root callers, but they must not own canonical particle loading/publication business logic or reintroduce root legacy `BrParticle` as the active publication schema.
...
- Final v1.2 documentation must keep the same ownership map: `:eyelib-particle` owns module APIs, `ParticleDefinition`, `ParticleDefinitionAdapter`, executable runtime, client integration, render manager, and loading/publication through `ParticleDefinitionRegistry` plus `ParticleResourcePublication`; root owns `BrParticleLoader`, compatibility adapters, `mc/impl/common/command`, `mc/impl/network/packet`, transport, and `NetClientHandlers` delegation; importer owns raw `io.github.tt432.eyelibimporter.particle.BrParticle`. Normal source tests must not read `.planning/` files; PFUT-02 packet-contract relocation, PFUT-03 independent particle artifact publication, unrelated fixture cleanup, and manual visual proof remain non-blocking scope boundaries recorded separately from automated JetBrains MCP gates.
...
- `src/main/java/io/github/tt432/eyelib/mc/impl/common/command/EyelibParticleCommand.java` owns Brigadier parsing, `ResourceLocationArgument`, `CommandSourceStack`, `ServerPlayer`, `Vec3Argument`, `Component`, and conversion from parsed `ResourceLocation` to string identifiers before request and packet seams.
- `src/main/java/io/github/tt432/eyelib/mc/impl/network/packet/` owns `SpawnParticlePacket(String spawnId, String particleId, Vector3f position)` and `RemoveParticlePacket(String removeId)` codec contracts.
- `src/main/java/io/github/tt432/eyelib/network/NetClientHandlers.java` remains context-free and delegates spawn/remove into `ParticleSpawnService`; `ParticleSpawnService` converts packets into module `ParticleSpawnRequest` and delegates runtime work into module APIs/client services.
- Permanent packet-contract relocation remains PFUT-02 scope, and broad ClientSmoke/hardware visual evidence remains Phase 14 scope.
