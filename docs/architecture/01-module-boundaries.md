...
| `:eyelib-particle/**` | `client.particle.module` | Own particle module contracts, `io.github.tt432.eyelibparticle.runtime.ParticleDefinition` as the canonical module runtime definition owner, `ParticleDefinitionAdapter` as the named conversion seam from importer-owned schema, executable runtime components/lifecycle, render-manager behavior, render adapter behavior, and `Dist.CLIENT` client hook integration; root may consume it, but it must not depend on root runtime packages, root managers, root registries, root packets, root capability helpers, or root mc/impl; `io.github.tt432.eyelibimporter.particle.BrParticle` is the canonical raw Bedrock particle schema/codec owner, the allowed particle -> importer dependency for ParticleDefinitionAdapter is the only schema dependency direction, retained root particle classes are transitional compatibility adapters only, and root `client/particle/bedrock/BrParticle` remains legacy/non-canonical until Phase 12 loading/publication rewires it away; Minecraft/Forge client rendering and `:eyelib-material` render-type resolution are allowed only in the documented particle `client/**` integration layer; Phase 13 rewires command/network integration, and Phase 14 owns broad/client verification evidence |
