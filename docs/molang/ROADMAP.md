...
| Phase 4 - Host and query bridge | `Current / near-done` | `mapping/api/` ports + HostRole<T> migration complete (98ed1de9). 6 contract tests green: host publication determinism, callable discovery roles, query variant selection matrix, callable variant ambiguity, callable publication signature roles, animation-clock transitional parity. Bind-link pass superseded by Phase 7 unified runtime resolution (MolangRuntimeSupport). | Maintain mapping registry stability; add query/callable registrations through documented API. |
