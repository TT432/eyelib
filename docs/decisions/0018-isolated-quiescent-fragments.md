...
| I-2 无服务定位 | 禁止可变全局服务定位器（含 `Manager.INSTANCE` 这种持有可变状态的 singleton）、`getInstance()`、副作用型 `Class.forName(...)`；不可变 null object 值（如 `MolangNull.INSTANCE`）允许 | ArchUnit + review（首次跑全部记入 baseline，P3 阶段 review 分类：服务定位器型必删、null object 型可保留） |
...
| **P3 Registry 替换 Manager.INSTANCE** | 8 个 `Manager` 子类逐个迁移到 `Registry`，删除 singleton；删除 `ManagerEventPublishBridge`；review baseline 中所有 domain INSTANCE 字段，服务定位器型必删、null object 型（如 `MolangNull.INSTANCE` / `EmptyComponent.INSTANCE`）可保留 | ArchUnit `domainMustNotUseSingletonInstance` baseline 收紧至仅剩 null object 型（服务定位器型 INSTANCE 全部归零） | 分子批，每批一 PR；P3.a / P3.b ... |
...
|---|---|---|---|
| `domainMustNotUseSingletonInstance` | Domain 包的 `public static final Xxx INSTANCE =` 字段（首次跑全部记入 baseline；P3 阶段 review 分类：服务定位器型必删、null object 型可保留） | I-2 | 9 处（AnimationManager / MolangMappingTree / HandwrittenMolangAstParserFrontend / MolangNull / EmptyComponent / InsideBlockNotifier / SuspectTracking / VibrationListener / VibrationDamper） |
| `applicationMustNotHaveStaticInitializer` | Application 包（`client/` / `common/` 等）不得出现 `static {` 业务 wiring | Q-2 | baseline 记录（EntityRenderOrchestrator / ClientBootstrap / ActiveModelRenderVisitors 等） |
| `applicationMustNotDependOnBridgeConcreteClasses` | Application 不得依赖 ACL（bridge）的具体类；仅允许依赖：Port 接口（`*Port.java`）+ 反射调度注解（`On*.java`）+ domain 层契约（如 `*Discovery.java` `@FunctionalInterface` 接口）；对齐 `molang/mapping/` 范式；同时禁止直接 import `net.minecraft.*` / `net.minecraftforge.*` / `net.neoforged.*` / `com.mojang.blaze3d.*` | **I-5** + ADR-0016 §5 | baseline 记录（EntityRenderOrchestrator 当前直接调 EntityRenderSystem 静态方法；helper 多处） |
...
| `noLazyInitScatter` | `getOwner() != entity` 等 init 守卫只能出现在状态所有者类内部 | Q-4 | baseline 记录（5 处散落：EntityRenderOrchestrator ×4 + ItemTrackRenderCache ×1） |
...
- I-2 服务定位器型零违规：domain 层无服务定位器型 INSTANCE（不可变 null object 型允许，由 P3 阶段 review 区分；ArchUnit baseline 区分服务定位器型 vs null object 型）；
